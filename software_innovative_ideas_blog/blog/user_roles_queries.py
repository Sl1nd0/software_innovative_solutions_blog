...
def get_user_role_by_user_id_query(userid): 

	result = {"success": False, "result": {}, "error": []}

	try:
		user_result = Users.objects.filter(id=userid).first()
		user_role_result = user_result.User_roleID
		print("user_role")
		print(user_role_result)
		...
		return result
			...
			print("get_user_role_by_user_id_query error")
			print(error)
			result["success"] = False
			result["result"] = userid
			result["error"].append("role doesn't exist for user_id {0}" .format(userid))
	return result
